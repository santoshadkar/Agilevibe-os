...
export default function handler(req, res) {
  ...
  const articlesWithTodayDate = CURATED_SEED.map((art, idx) => ({
    ...art,
    publishedAt: new Date(Date.now() - idx * 1000 * 60 * 35).toISOString()
  }));
  ...
  return res.status(200).json({
    ...
    data: {
      lastRunAt: nowIso,
      batchStatus: 'SUCCESS',
      totalArticles: articlesWithTodayDate.length,
      countsByCategory: {
        ai: articlesWithTodayDate.filter(a => a.category === 'ai').length,
        cybersecurity: articlesWithTodayDate.filter(a => a.category === 'cybersecurity').length,
        security: articlesWithTodayDate.filter(a => a.category === 'security').length,
        agile: articlesWithTodayDate.filter(a => a.category === 'agile').length,
        cloud: articlesWithTodayDate.filter(a => a.category === 'cloud').length,
        governance: articlesWithTodayDate.filter(a => a.category === 'governance').length,
        data: articlesWithTodayDate.filter(a => a.category === 'data').length,
        quantum: articlesWithTodayDate.filter(a => a.category === 'quantum').length,
        greentech: articlesWithTodayDate.filter(a => a.category === 'greentech').length,
      },
      morningTLDR: [
      ...
      articles: articlesWithTodayDate
    }
  });
}
