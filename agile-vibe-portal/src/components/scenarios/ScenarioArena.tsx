...
import { EXPANDED_SCENARIOS } from '../../data/scenarioBank';
...
export const ScenarioArena: React.FC = () => {
  ...
  const [selectedScenarioId, setSelectedScenarioId] = useState<string>(EXPANDED_SCENARIOS[0].id);
  const [selectedOptionId, setSelectedOptionId] = useState<string | null>(null);
  const [userRationale, setUserRationale] = useState<string>('');
  const [hasSubmitted, setHasSubmitted] = useState<boolean>(false);

  const filteredScenarios = (roleFilter as string) === 'all'
    ? EXPANDED_SCENARIOS
    : EXPANDED_SCENARIOS.filter(s => s.targetRole === roleFilter);

  const currentScenario = filteredScenarios.find(s => s.id === selectedScenarioId) || filteredScenarios[0] || EXPANDED_SCENARIOS[0];
  ...
  const handleNextScenario = () => {
    ...
    if (currentIndex < filteredScenarios.length - 1) {
      const nextS = filteredScenarios[currentIndex + 1];
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/40">
                  {filteredScenarios.length} Interactive Challenges
                </span>
              </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2">
        <button
          onClick={() => { setRoleFilter('all'); setSelectedOptionId(null); setHasSubmitted(false); }}
        ...
        >
          All Roles ({EXPANDED_SCENARIOS.length})
        </button>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 glass-card p-5 rounded-2xl border border-slate-800 space-y-3">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Select Scenario Arena Challenge:</h3>
